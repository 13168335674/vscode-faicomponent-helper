...
export function encodeDocsUri(query?: Query): Uri {
  return Uri.parse(`faicomponent-helper://search?${JSON.stringify(query)}`);
}
...
export class App {
  ...
  openDocs(query?: Query, title = 'faicomponent-helper', editor = window.activeTextEditor) {
  ...
  getWebviewContent(query: Query) {
    const config = workspace.getConfiguration('faicomponent-helper');
...
export class FaicomponentDocsContentProvider implements TextDocumentContentProvider {
...
export class FaicomponentCompletionItemProvider implements CompletionItemProvider {
  ...
  provideCompletionItems(
    ...
    if (this.isAttrValueStart(tag, attr)) {
      console.log(`isAttrValueStart`, this.getAttrValueSuggestion(tag.text, attr));
      ...
      switch (document.languageId) {
        ...
        case 'html':
          // todo
          console.log(`this.getTagSuggestion();`, this.getTagSuggestion());
